...
class SfMLearner(object):
    ...
    def build_train_graph(self):
        ...
        with tf.name_scope("data_loading"):
            ...
            # intrinsics.set_shape([opt.num_scales, 3, 3])
            intrinsic_mat_inv = tf.matrix_inverse(intrinsics)
            tgt_image = self.preprocess_image(tgt_image)
            src_image_stack = self.preprocess_image(src_image_stack)

        with tf.name_scope("depth_prediction"):
            input_img=tf.concat([tgt_image,src_image_stack[:,:,:,3:6]],axis=0)

            pred_disp, src_pred_disp,depth_net_endpoints = disp_net(input_img,
                                                      is_training=True)
            ...
            src_pred_depth=[1. / d1 for d1 in src_pred_disp]

        #
        # with tf.name_scope("depth_prediction"):
        #     input_img=np.append(tgt_image,src_image_stack[:,:,:,3:6],axis=0);
        #     pred_disp, depth_net_endpoints = disp_net(tgt_image,
        #                                               is_training=True)
        #     pred_depth = [1. / d for d in pred_disp]
        # # with tf.name_scope("depth_prediction_2"):
        #     src_pred_disp,src_net_endpoints=disp_net2(src_image_stack[:,:,:,3:6],
        #                                               is_training=False)
        #     src_pred_depth=[1. / d1 for d1 in src_pred_disp]
        ...
        #with tf.name_scope("src_depth_prediction"):
            #for i in range(3):
            #     for s in range(len(src_pred_depth)):
            #         tgt_cloud = get_cloud(pred_depth[s],
            #                                       intrinsic_mat_inv[:, s, :, :],
            #                                       name='cloud%d_%d' % (s, i))
            #         src_cloud = get_cloud(src_pred_depth[s],
            #                                       intrinsic_mat_inv[:, s, :, :],
            #                                       name='cloud%d_%d' % (s, i))
        #tf.reset_default_graph()
        ...
        with tf.name_scope("compute_loss"):
            ...
            for s in range(opt.num_scales):
                ...
                for i in range(opt.num_source):
                    # Inverse warp the source image to the target image frame 在这里对源图像进行操作
                    curr_proj_image, tgt_cloud,src_cloud = projective_inverse_warp(
                        curr_src_image_stack[:, :, :, 3 * i:3 * (i + 1)],
                        tf.squeeze(pred_depth[s], axis=3),
                        tf.squeeze(src_pred_depth[s],axis=3),
                        pred_poses[:, i, :],
                        intrinsics[:, s, :, :])
                    curr_proj_error = tf.abs(curr_proj_image - curr_tgt_image)
                    if self.icp_weight > 0:
                        # Compute ICp loss V2.
                        # tgt_depth = pred_depth      could[i]？
                        # src_depth =   这个如何调用... cloud[i+1]?
                        # tgt_pcl = project.get_cloud(tgt_depth, intrinsic_mat_inv)
                        # src_pcl = project.get_cloud(src_depth intrinsic_mat_inv)
                        pose_mat = pose_vec2mat(pred_poses[:, 1, :])
                        tgt_pcl = tgt_cloud
                        src_pcl = src_cloud


                        dims = tgt_pcl.shape.as_list()
                        dims0,dims1,dims2,dims3=dims[0],dims[1],dims[2],dims[3]
                        reshape_tgt_pcl= tf.reshape(tgt_pcl, [dims0,dims1,dims2*dims3])
                        t2s_pcl = tf.matmul(pose_mat, reshape_tgt_pcl)[:, 0:3, :]

                        # t2s_pcl = tf.reshape(t2s_pcl, [dims0,3,dims2,dims3])
                        # t2s_pcl = tf.matmul(pose_mat, tgt_pcl)[:, 0:3, :]

                        tgt_pcl = tf.reshape(tgt_pcl, [dims0,dims1,dims2*dims3])
                        src_pcl = tf.reshape(src_pcl, [dims0, dims1, dims2 * dims3])
                        centroids1 = tf.reduce_mean(
                            t2s_pcl, axis=1, keep_dims=True)
                        centroids2 = tf.reduce_mean(
                            src_pcl, axis=1, keep_dims=True)


                        #cc=tgt_pcl - centroids1
                        cam_coords1_shifted = tf.expand_dims(
                            tf.transpose(tgt_pcl - centroids1, [0, 2, 1]), -1)
                        cam_coords2_shifted = tf.expand_dims(
                            tf.transpose(src_pcl - centroids2, [0, 2, 1]), -2)
                        distance = tf.reduce_sum(
                            tf.matmul(cam_coords1_shifted, cam_coords2_shifted),
                            axis=1,
                            keep_dims=False)
                        icp_loss, indice = tf.nn.top_k(distance, k=1, sorted=False)

                    # explainability prediction 用于建立假设中的训练序列
                    if opt.explain_reg_weight > 0:
            ...
            icp_loss=tf.reduce_mean(icp_loss)
            total_loss = pixel_loss + smooth_loss + exp_loss + self.icp_weight * icp_loss
    ...
    def collect_summaries(self):
        ...
        tf.summary.scalar("icp_loss", self.icp_loss)
    ...
    def build_depth_test_graph(self):
        ...
        input=tf.concat([input_mc,input_mc],axis=0)
        with tf.name_scope("depth_prediction"):
            pred_disp, src_pred_disp, depth_net_endpoints = disp_net(
